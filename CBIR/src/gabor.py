...
class Gabor(object):  
  ...
  def make_samples(self, db, dbName, verbose=True):
    if h_type == 'global':
      sample_cache = "gabor-{}-theta{}-frequency{}-sigma{}-bandwidth{}".format(h_type, theta, frequency, sigma, bandwidth, dbName)
    elif h_type == 'region':
      sample_cache = "gabor-{}-n_slice{}-theta{}-frequency{}-sigma{}-bandwidth{}".format(h_type, n_slice, theta, frequency, sigma, bandwidth, dbName)
...
if __name__ == "__main__":
    ...
    # evaluate database
    APs, prevision = my_evaluate_class(db_train, db_test, f_class=Gabor, d_type=d_type, depth=depth)
    cls_MAPs = []
